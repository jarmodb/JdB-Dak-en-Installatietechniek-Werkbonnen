...
// Ondersteunt zowel de nieuwe `arbeidsposten`-lijst (omschrijving + uren) als het oude
// losse `uren`-veld van offertes die nog niet gemigreerd zijn naar de lijstvorm
function arbeidspostenVan(form) {
  const posten = (form.arbeidsposten || []).filter(a => a.omschrijving || (parseFloat(a.uren) || 0) > 0)
  if (posten.length) return posten
  if (form.uren && parseFloat(form.uren) > 0) return [{ omschrijving: 'Werkzaamheden', uren: form.uren }]
  return []
}
...
function berekenTotalen(form) {
  ...
  const btw_bedrag = subtotaalNaKorting * (btw / 100)
  return { arbeidskosten, totaalUren, subtotaalMat, subtotaal, korting_bedrag, subtotaalNaKorting, btw_bedrag, totaal: subtotaalNaKorting + btw_bedrag }
}
...
export async function maakOffertePdf(offerte, instellingen = {}) {
  ...
  const topMarge = 18
  const bodemMarge = 18
  const maxY = H - bodemMarge

  // Begint een nieuwe pagina en zet de cursor terug naar de bovenmarge —
  // nodig omdat een offerte met veel tekst/materialen meerdere pagina's beslaat.
  function nieuwePagina() {
    doc.addPage()
    y = topMarge
  }
  // Controleert of er nog `hoogte` mm ruimte over is op de huidige pagina;
  // zo niet, begin een nieuwe pagina.
  function ruimteCheck(hoogte) {
    if (y + hoogte > maxY) nieuwePagina()
  }
  ...
  if (offerte.tekst?.trim()) {
    ...
    // Regel voor regel tekenen i.p.v. in één keer, zodat lange offertetekst
    // automatisch op een nieuwe pagina doorloopt i.p.v. van de pagina af te lopen
    lines.forEach((regel) => {
      ruimteCheck(regelH)
      txt(regel, ml, y)
      y += regelH
  ...
  // ── SPECIFICATIE WERKZAAMHEDEN (uitsplitsing van de uren) ───────────
  const totalen = berekenTotalen(offerte)
  const arbeidsposten = (offerte.arbeidsposten || []).filter(a => a.omschrijving)
  if (arbeidsposten.length > 0) {
    const rh = 7
    const c1 = ml + 2
    const c5 = ml + cw - 2

    const tekenArbeidskop = () => {
      fill(THBG)
      rect(ml, y, cw, rh, 'F')
      draw(THBORD); lw(0.4)
      ...
      txt('Uren', c5, y + 4.7, { align: 'right' })
      y += rh
    }

    ruimteCheck(6 + rh * 2)
    fnt('bold', 10); tc([68, 68, 68])
    txt('Specificatie werkzaamheden', ml, y)
    y += 6

    tekenArbeidskop()

    arbeidsposten.forEach((a, i) => {
      if (y + rh > maxY) {
        nieuwePagina()
        tekenArbeidskop()
      }
      fnt('normal', 8.5); tc(TEXT)
      txt(a.omschrijving, c1, y + 4.7)
      txt(`${a.uren || 0} uur`, c5, y + 4.7, { align: 'right' })
      if (i < arbeidsposten.length - 1) {
        draw(ROWBORD); lw(0.3)
        line(ml, y + rh, ml + cw, y + rh)
      }
      y += rh
  ...
  if (offerte.materialen_tonen && mats.length > 0) {
    ...
    // Tekent de tabelkop (licht grijs, geen donkere balk) op de huidige y en geeft de nieuwe y terug —
    // wordt ook opnieuw aangeroepen bovenaan een nieuwe pagina zodat de tabel leesbaar blijft
    const tekenTabelkop = () => {
      fill(THBG)
      rect(ml, y, cw, rh, 'F')
      draw(THBORD); lw(0.4)
    ...
    mats.forEach((m, i) => {
      // Past de rij niet meer op deze pagina? Begin een nieuwe pagina en herhaal de tabelkop
      if (y + rh > maxY) {
        nieuwePagina()
        tekenTabelkop()
      }
  ...
  const totaalRows = [
    totalen.arbeidskosten > 0 ? [`Arbeid (${totalen.totaalUren} uur × ${euro(offerte.uurtarief)})`, totalen.arbeidskosten] : null,
    totalen.subtotaalMat > 0 && totalen.arbeidskosten > 0 ? ['Materialen', totalen.subtotaalMat] : null,
    ['Subtotaal (excl. BTW)', totalen.subtotaal],
    totalen.korting_bedrag > 0 ? [`Korting${offerte.korting_type === 'percentage' ? ` (${parseFloat(offerte.korting_waarde) || 0}%)` : ''}`, -totalen.korting_bedrag] : null,
    [`BTW (${offerte.btw_percentage ?? 21}%)`, totalen.btw_bedrag],
  ].filter(Boolean)

  // Houd het hele totalenblok (scheidingslijnen + regels + eindtotaal) bij elkaar op één pagina
  ruimteCheck(7 + totaalRows.length * 6.2 + 8 + 10)
  ...
  totaalRows.forEach(([label, val]) => {
    ...
    txt(label, ml, y + 4)
    txt(val < 0 ? `− ${euro(Math.abs(val))}` : euro(val), ml + cw, y + 4, { align: 'right' })
    y += 6.2
  })
  ...
  if (offerte.notities?.trim()) {
    ...
    ruimteCheck(6 + notitieLines.length * 4.4 + 6)
    draw(ROWBORD); lw(0.3)
    line(ml, y, ml + cw, y)
    y += 6
  ...
  // Het slotblok wordt het liefst onderaan de pagina geplaatst (zoals een echte brief),
  // maar als het daar niet meer past, begint het op een nieuwe pagina i.p.v. af te breken
  fnt('normal', 9)
  ...
  const footerBlokHoogte = afsluit.length * 4.4 + 10 + 16 + 6
  let footerY = Math.max(y + 8, H - 56)
  if (footerY + footerBlokHoogte > maxY) {
    nieuwePagina()
    footerY = Math.max(y + 8, H - 56)
  }
  tc(GRAY2)
