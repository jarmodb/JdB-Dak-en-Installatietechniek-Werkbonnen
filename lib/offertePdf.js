...
export async function maakOffertePdf(offerte, instellingen = {}) {
  ...
  txt(`Nummer: ${offerte.nummer}`, W - mr, numY, { align: 'right' }); numY += 4.5
  txt(`Datum: ${datumNL(offerte.datum)}`, W - mr, numY, { align: 'right' }); numY += 4.5
  ...
  const headerBottom = Math.max(subY, numY, headerTopY + (logo ? 16 : 6)) + 5
  draw(GOLD); lw(0.7)
  ...
  // ── OFFERTETEKST (gouden linkerrand, geen vulling — zoals origineel) ─
  if (offerte.tekst?.trim()) {
    ...
    fnt('normal', 9); tc(TEXT)
    const lines = doc.splitTextToSize(tekst, cw - 12)
    const regelH = 4.6   // mm per regel — komt overeen met line-height 1.7 op 9pt tekst
    const tekstH = lines.length * regelH

    fill(GOLD); rect(ml, y, 0.9, tekstH + 2, 'F')
    lines.forEach((regel, i) => txt(regel, ml + 8, y + 5 + i * regelH))
    y += tekstH + 10
  }
  ...
  // ── FOOTER (afsluittekst, bedrijfsgegevens, akkoord-vakken) ─────────
  ...
  if (bedrijfsgegevens.length) {
    ...
    txt(bedrijfsgegevens.join('  ·  '), ml, footerY)
    footerY += 5
  }

  // Akkoord-vakken: Naam / Datum / Handtekening
  footerY += 6
  const akkoordItems = ['Naam', 'Datum', 'Handtekening']
  const lijnW = (cw - 16) / 3
  akkoordItems.forEach((label, i) => {
    const x = ml + i * (lijnW + 8)
    draw([51, 51, 51]); lw(0.3)
    line(x, footerY + 9, x + lijnW, footerY + 9)
    fnt('normal', 7.5); tc(GRAY)
    txt(label, x, footerY + 13.5)
  })
