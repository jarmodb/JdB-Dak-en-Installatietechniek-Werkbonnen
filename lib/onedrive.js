...
// Synchroniseert foto's met een Supabase URL naar OneDrive (stil op de achtergrond)
export async function syncFotosNaarOneDrive(fotos, werkbonNummer, klantNaam) {
  if (!fotos?.length) return
  const supabaseFotos = fotos.filter(f => f.url && f.url.includes('supabase'))
  if (!supabaseFotos.length) return

  let token
  try { token = await getToken() } catch { return } // niet ingelogd, stil falen

  for (const foto of supabaseFotos) {
    try {
      const res = await fetch(foto.url)
      if (!res.ok) continue
      const blob = await res.blob()
      const url = graphPad('Werkbonnen', mapNaam(werkbonNummer, klantNaam), foto.naam)
      await fetch(url, {
        method: 'PUT',
